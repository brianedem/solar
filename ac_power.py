...
(options, leftover) = parser.parse_args()
    ...
    # connect to the battery current monitor
adc = AD7705.adc(0,0)
adc_reference = 2.5
adc_bits = 16
battery_divider = 4.95/1.63 # 4.95V supplied to resistive divider provided ADC with 1.63V
bus_voltage = 400
#battery_monitor_zero = 5.007
battery_monitor_zero = 5.018

meter_peak = 0.0
while True :
    battery_adc = adc.read_data(0, unipolar=True)
    battery_i_monitor = battery_divider * battery_adc/(2**adc_bits) * adc_reference
    battery_current = -(battery_monitor_zero-battery_i_monitor)/2.0 * 50
    battery_power = int(battery_current*bus_voltage)
        ...
        if map(ord,response[0:3])==[1,4,20] :
            if map(ord,response[-2:]) == crc16(map(ord,response)) :
                ...
                if options.dump :
                    ...
                    print "solar = ", power + battery_power
                    print
                ...
                if options.log :
                    ...
                    out_file.write(timestamp+",%.1f,%.3f,%.1f,%d,%.1f,%.2f,%d\n" %
                            (voltage, current, power, energy, frequency, pwr_fctr, battery_power))
